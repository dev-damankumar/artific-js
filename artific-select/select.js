				...
				for (const mutation of mutationsList) {
					if (mutation.type === 'childList') {
						...
						if (multiple) {
							...
							selectDiv.querySelector(".select-span").innerHTML = "<span class='default-span'>Choose Option</span>"
						}
		...
		this.select = function (styleTheme) {
			...
			if (this.selectTags.length > 0) {
				...
				this.selectTags.forEach(selectTag => {
					if (instances.includes(selectTag)) return console.error("select has already been applied")
					instances.push(selectTag)
					...
					if (optionsDiv) {
						...
						if (styleTheme && typeof styleTheme === "object") {
							selectDiv.className = `select-div ${multiple ? "select-div-multiple" : ""} ${styleThemeClasses}`
						} else {
							selectDiv.className = `select-div ${multiple ? "select-div-multiple" : ""}`
						}
						...
						selectDiv.innerHTML += "<span class='select-span'>" + `${multiple ? "<span class='default-span'>Choose Option</span>" : firstOption}` + "</span>"
